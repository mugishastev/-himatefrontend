...
export const InfoSidebar: React.FC = () => {
    ...
    return (
        <aside className="w-[400px] h-full bg-[#111b21] border-l border-[#2a3942] flex flex-col shrink-0 animate-in slide-in-from-right duration-300">
            <header className="h-[60px] bg-[#202c33] flex items-center px-6 shrink-0 border-b border-[#2a3942]">
                <button
                    onClick={() => setInfoPane(false)}
                    className="mr-6 text-[#aebac1] hover:text-[#e9edef] transition-colors"
                >
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.4" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
                <h3 className="font-medium text-[#e9edef] text-lg">
                    {infoPaneType === 'GROUP' ? 'Group info' : 'Contact info'}
                </h3>
            </header>

            <div className="flex-1 overflow-y-auto no-scrollbar bg-[#111b21] space-y-3 pb-10">
                {/* Profile Section */}
                <div className="bg-[#202c33] p-8 flex flex-col items-center shadow-sm">
                    ...
                    <h2 className="text-2xl font-normal text-[#e9edef] mb-1">{displayName}</h2>
                    {!activeConversation.isGroup && (
                        <p className="text-[#8696a0] text-lg font-normal">
                ...
                <div className="bg-[#202c33] p-6 shadow-sm">
                    <h4 className="text-[#8696a0] text-sm font-medium mb-4 uppercase tracking-wide">
                ...
                {activeConversation.isGroup && (
                    <div className="bg-[#202c33] shadow-sm flex-1">
                        <div className="p-6 pb-2">
                            <h4 className="text-[#8696a0] text-sm font-medium uppercase tracking-wide">
                                {activeConversation.participants.length} Participants
                            </h4>
                        </div>
                        <div className="divide-y divide-[#2a3942]">
                            {activeConversation.participants.map((p) => (
                                <div key={p.id} className="p-4 px-6 flex items-center gap-4 hover:bg-[#2a3942] transition-colors cursor-pointer">
                                    <div className="w-10 h-10 rounded-full overflow-hidden shrink-0">
                                        <UserAvatar user={p.user} size="sm" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                            <h5 className="font-medium text-[#e9edef] truncate">
                ...
                <div className="bg-[#202c33] p-6 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[#8696a0] text-sm font-medium uppercase tracking-wide">Media, links and docs</h4>
                        <span className="text-xs text-[#8696a0] cursor-pointer hover:underline">0 {'>'}</span>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                        <div className="aspect-square bg-[#2a3942] rounded flex items-center justify-center text-[#8696a0]">
                ...
                <div className="bg-[#202c33] shadow-sm">
                    <button className="w-full text-left p-6 flex items-center gap-6 text-[#f15c6d] hover:bg-[#2a3942] transition-colors">
                    ...
                    <button className="w-full text-left p-6 flex items-center gap-6 text-[#f15c6d] hover:bg-[#2a3942] transition-colors border-t border-[#2a3942]">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        <span className="font-normal">{activeConversation.isGroup ? 'Report group' : 'Report contact'}</span>
                    </button>
                </div>
            </div>
        </aside>
