...
export const AdminUsersPage: React.FC = () => {
    ...
    const fetchUsers = () => {
        setLoading(true);
        const isBanned = filterStatus === 'ALL' ? undefined : filterStatus === 'BANNED';
        adminApi.getUsers(page, 20, search || undefined, isBanned)
            .then((res) => {
                setData(res);
                setLoading(false);
            })
        ...
        <div className="p-4 sm:p-8 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">{data?.total?.toLocaleString() ?? '–'} total users match criteria</p>
                </div>
                {/* Search */}
            ...
            <div className="flex items-center gap-2 border-b border-slate-850 pb-4 shrink-0">
                {(['ALL', 'ACTIVE', 'BANNED'] as const).map((status) => (
                    <button
                        key={status}
                        onClick={() => { setPage(1); setFilterStatus(status); }}
                        className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${filterStatus === status ? 'bg-brand/15 text-brand border-brand/35' : 'bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-700 hover:text-white'}`}
                    >
                        {status === 'ALL' ? 'All Users' : status === 'ACTIVE' ? 'Active' : 'Banned / Suspended'}
                    </button>
                ))}
            </div>
            ...
            {banModalVisible && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-scale-up">
                        ...
                        <p className="text-sm text-slate-400 mb-6">This will immediately log the user out and prevent them from accessing the platform. An audit event will be recorded.</p>

                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-350 mb-1.5">Reason for Suspension</label>
                                <textarea
                                    value={banReason}
                                    onChange={(e) => setBanReason(e.target.value)}
                                    placeholder="Please provide a clear reason for the audit trail..."
