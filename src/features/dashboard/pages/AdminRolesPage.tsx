...
const PERMISSION_LABELS: Record<string, string> = {
    ...
    can_suspend_users: 'Suspend/Ban Users',
    can_change_system_settings: 'Platform Configuration',
};

const DEFAULT_PERMISSIONS = ['can_view_audit_logs', 'can_delete_messages', 'can_suspend_users', 'can_change_system_settings'];
...
export const AdminRolesPage: React.FC = () => {
    ...
    const allActions = useMemo(() => {
        ...
        DEFAULT_PERMISSIONS.forEach((a) => actions.add(a));
    ...
    return (
        <div className="p-4 sm:p-8">
            <h1 className="text-2xl font-bold text-white mb-2">Admin Roles & Permissions</h1>
            <p className="text-slate-400 text-sm mb-8">Define administrative role-based access control (RBAC) scopes and toggle permission matrices securely.</p>

            <div className="flex flex-col md:flex-row gap-6">
                {/* Role List */}
                <div className="w-full md:w-72 flex-shrink-0 space-y-3">
                    {loading ? (
                        <div className="text-sm text-slate-500">Loading platform roles...</div>
                    ) : roles.map((role) => (
                        <button
                            key={role.id}
                        ...
                        >
                            <div className="flex items-center justify-between mb-1.5">
                                ...
                                <span className="text-[10px] text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{(role.permissions ?? []).length} active</span>
                            </div>
                            <p className="text-xs text-slate-500 line-clamp-2">Scope level: {role.name === 'ADMIN' ? 'Full Super Admin Access' : 'Custom Permission Scope'}</p>
                        </button>
                    ))}
                ...
                <div className="flex-1 bg-slate-900 border border-slate-800 rounded-2xl p-6">
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-slate-400 mt-1">Role authorization permission matrix</p>
                        </div>
                        {!isEditing && selectedRole && (
                            <button onClick={handleEditClick} className="px-4 py-2 bg-brand/10 hover:bg-brand/20 text-brand border border-brand/25 rounded-lg text-sm font-medium transition-colors">
                                Edit Permission Matrix
                            </button>
                        )}
                    </div>

                    <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">RBAC Scopes</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {allActions.map((perm) => {
                            ...
                            return (
                                <label
                                    key={perm}
                                    className={`flex items-center gap-3 p-4 rounded-xl border transition-all ${isEditing ? 'cursor-pointer' : 'cursor-default'} ${active ? 'border-brand/35 bg-brand/5' : 'border-slate-800 bg-slate-950/30'}`}
                                >
                    ...
                    {isEditing && (
                        <div className="flex gap-3 mt-6">
                            ...
                            <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-brand text-white hover:bg-brand/90 transition-colors disabled:opacity-60 shadow-lg shadow-brand/20">
                                {saving ? 'Saving Scope changes...' : 'Save Matrix Changes'}
                            </button>
                        </div>
                    )}
                </div>
